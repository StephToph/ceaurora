...
<?php echo form_open_multipart($form_link, array('id'=>'bb_ajax_form', 'class'=>'')); ?>
    ...
    <?php if($param2 == 'delete') { ?>
        ...
        <div class="row">
            <div class="col-sm-12 text-center">
                ...
                <input type="hidden" name="d_field_id" value="<?php if(!empty($d_id)){echo $d_id;} ?>" />
            </div>
    ...
    <?php if($param2 == 'edit' || $param2 == '') { ?>
        ...
        <div class="row">
            <input type="hidden" name="field_id" value="<?php if(!empty($e_id)){echo $e_id;} ?>" />
            ...
            ?>
                ...
                <?php if(!empty($e_territory)){?>
                    <div class="col-sm-6 mb-3">
                        <div class="form-group">
                            ...
                            <select id="territorys" name="territory" class="js-select2" required  data-search="on"  onchange="master();" >
                                <option value="0" selected><?=translate_phrase('All Territory'); ?>...</option>
                                <?php 
                                    foreach($states as $s) {
                                        $s_sel = '';
                                        if(!empty($e_territory)) {
                                            if($e_territory == $s->id) { $s_sel = 'selected'; }
                                        }
                                        echo '<option value="'.$s->id.'" '.$s_sel.'>'.$s->name.'</option>';
                                    }
                                ?>
                ...
                <?php } else{?>
                    <div class="col-sm-6 mb-3">
                        <div class="form-group" id="">
                            ...
                            <select class="form-select js-select2" data-search="on" id="territorys" name="territory"  onchange="master();" required>
                ...
                <div class="col-sm-6 mb-3">
                    <div class="form-group">
                        <label for="name">*<?=translate_phrase('Task Master');?></label>
                        <select class="form-select js-select2" data-search="on" id="master_id" name="master_id" required>
                            <option value=""><?=translate_phrase('Select Territory First'); ?></option>
                            
                        </select>
                    </div>
                </div>
            <?php } else{ 
                $territory = $this->Crud->read_field('id', $log_id, 'user', 'territory');
                $lga_id = $this->Crud->read_field('id', $log_id, 'user', 'lga_id');
                echo '<input type="hidden" name="master_id" value="'.$log_id.'">
                <input type="hidden" name="territory" value="'.$territory.'">
                <input type="hidden" name="lga_id" value="'.$lga_id.'">';
            }?>
...
<script>
    ...
    function master(man){
        var territory = $('#territorys').val();
        $.ajax({
            url: site_url + 'accounts/validate_field/' + territory,
            data:{man:man},
            success: function(data) {
                $('#master_id').html(data);
            }
